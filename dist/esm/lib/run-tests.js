...
async function runTests(serverless, options, conf) {
  const functionName = options.f || options.function;
  const allFunctions = serverless.service.getAllFunctions();
  const config = Object.assign({
    testEnvironment: "node"
  }, conf);
  allFunctions.forEach(name => (0, _utils.setEnv)(serverless, name));
  if (!config.testRegex) {
    if (functionName) {
      if (allFunctions.indexOf(functionName) >= 0) {
        (0, _utils.setEnv)(serverless, functionName);
        Object.assign(config, {
          testRegex: `${functionName}\\.test\\.[jt]s$`
        });
      } else {
  ...
  return (0, _jest.runCLI)(config, [serverless.config.servicePath]).then(output => {
